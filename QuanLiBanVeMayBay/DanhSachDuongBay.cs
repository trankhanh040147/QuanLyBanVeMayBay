...
namespace QuanLyBanVeMayBay
{
    public partial class DanhSachDuongBay : Form
    {
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                DialogResult tl = MessageBox.Show("Bạn muốn xóa đường bay khỏi CSDL?", "Xóa",
                       MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (tl == DialogResult.Yes)
                {
                    DuongBay temp_db = new DuongBay();
                    temp_db.MaDuongBay = txtTim.Text;
                    if (!db.deleteDuongBay(ref err, temp_db))
        ...
        {
            CapNhatDuongBay cndb = new CapNhatDuongBay();
            cndb.Show();
        }
