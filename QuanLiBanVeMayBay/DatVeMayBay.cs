...
namespace QuanLyBanVeMayBay
{
    public partial class DatVeMayBay : Form
    {
        ...
        public DatVeMayBay(string TenDangNhap)
        {
            InitializeComponent();
        ...
        private void btnHoanTat_Click(object sender, EventArgs e)
        {
            ...
            var checkKH = dbs.KhachHangs.FirstOrDefault(p => p.CMND == txtCMND.Text);
            bool check;
            if (checkKH != null)
                check = true;
            else
            ...
            if (check == false)
            {
                ...
                if (dbkh.insertKhachHang(ref err, kh))
                {
                    ...
                    var NhanVien = dbs.NhanViens.FirstOrDefault(p => p.TenDangNhap.Contains(TenDangNhap));
                    if (kh1 == null)
                    {
            ...
            else
            {
                ...
                if (kh1 != null)
                {
                    vb.MaVeBan = ThongTinVe.MaVe + kh1.MaKhachHang + NhanVien.MaNhanVien;
                    vb.MaKhachHang = kh1.MaKhachHang;
                    vb.MaVe = ThongTinVe.MaVe;
                    vb.TongTien = int.Parse(txtTongTien.Text);
                    vb.ThoiGianBan = DateTime.Now;
        ...
        private void DatVeMayBay_Load(object sender, EventArgs e)
        {
            ...
            lblNhanvienHoTro.Text = nv.TenNhanVien;
            lblNgayHienTai.Text = DateTime.Now.ToShortDateString();
            lblGioHienTai.Text = DateTime.Now.TimeOfDay.ToString();
        }
        ...
        private void cbMaChuyenBay_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            var ThongTinVe = dbs.ThongTinChiTietVes.Where(p => p.MaChuyenBay.Equals(ChuyenBay.MaChuyenBay)).Select(p => p);
            foreach (var i in ThongTinVe)
            {
                if (i.LoaiVe.Contains("1"))
                    txtVeLoai1ConLai.Text = i.SoLuongCon.ToString();
                else if (i.LoaiVe.Contains("2"))
